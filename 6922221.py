...
M=(w*(-6*x**2+6*L*x-L**2))/12 #bending moment M
V = w*(L/2 - x) #shear force V
#Q a
#Bending moment(M) and shear force(V) at the first end, x=0
m='The bending moment at x=0 is'
v='The shear force x=0 is'
print
print('(a.1)'+m+str(M)+'and',v+str(V))
#Bending moment(M) and shear force(V) at the first end, x=L
x = L
...
print()
print('(f) The points along L at which the absolute values of the bending moment array is minimum are 
{0} and {1}'.format(root_1f,root_2f)
#Q g
Let the relative errors be r_e
r_e1 = ((root_1b - root_1f)/root_1b*100) 
r_e2 = ((root_2f - root_2b)/root_2f*100)
print 
print('(g) The relative errors between estimated points of contra-flexure are {0}% and 
{1}%'.format(r_e1,r_e2)
#Q h
...
print 
print('(h.1) The points at which the maximum bending moment occur are {0} and 
...
root_2 = (-b + np.sqrt(discriminant))/2*a
print 
print('(h.2) The points at which the minimum bending moment occur are {0} and 
{1}'.format(root_1,root_2)
